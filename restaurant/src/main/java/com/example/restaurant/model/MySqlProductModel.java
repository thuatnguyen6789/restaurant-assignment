...
public class MySqlProductModel implements ProductModel {
    ...
    public Product findByName(String name) {
        ...
        try {
            Connection connection = ConnectionHelper.getConnection();
            String sqlQuery = "select * from products where status = ? and name = ?";
            PreparedStatement preparedStatement = connection.prepareStatement(sqlQuery);
            preparedStatement.setInt(1, CategoryStatus.ACTIVE.getValue());
    ...
    public Product findByCategoryId(int categoryId) {
        ...
        try {
            Connection connection = ConnectionHelper.getConnection();
            String sqlQuery = "select * from products where status = ? and categoryId = ?";
            PreparedStatement preparedStatement = connection.prepareStatement(sqlQuery);
            preparedStatement.setInt(1, CategoryStatus.ACTIVE.getValue());
    ...
    public Product update(int id, Product updateObj) {
        try {
            ...
            String sqlQuery = "update products " +
                    "set  name = ?, categoryId = ?, description = ?, detail = ?, thumbnail = ?, price = ?, createdAt = ?, updatedAt = ?, status = ? where id = ?";
            PreparedStatement preparedStatement = connection.prepareStatement(sqlQuery);
